...
int main() {
  ...
  const std::filesystem::path temp_dir =
      std::filesystem::temp_directory_path() / "exp_femto_3d_catalog_test";
  std::filesystem::create_directories(temp_dir);
  const std::string input_root = WriteToyInput(temp_dir / "input.root");
  const std::string config_path =
      WriteConfig(temp_dir / "config.toml", input_root, temp_dir.string());
  ...
  Expect(entries.front().slice_directory.rfind("slices/", 0) == 0,
         "slice directory should live under slices/");
  Expect(entries[0].cf_object_path.find("/CF3D") != std::string::npos,
         "catalog should store CF object path");
