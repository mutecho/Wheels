...
inline bool NearlyEqual(const double lhs,
                        const double rhs,
                        const double tolerance = 1.0e-6) {
  return std::abs(lhs - rhs) < tolerance;
}

inline bool MatchesRangeBin(const RangeBin& lhs,
                            const RangeBin& rhs,
                            const double tolerance = 1.0e-6) {
  return NearlyEqual(lhs.min, rhs.min, tolerance) &&
         NearlyEqual(lhs.max, rhs.max, tolerance);
}
