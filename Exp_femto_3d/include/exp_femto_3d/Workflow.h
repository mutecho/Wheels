...
[[nodiscard]] BuildCfRunStatistics RunBuildCf(const ApplicationConfig& config,
                                              const Logger& logger);

[[nodiscard]] FitRunStatistics RunFit(
    const ApplicationConfig& config,
    const Logger& logger,
    std::optional<FitModel> override_model = std::nullopt,
    std::optional<std::string> input_cf_root_path = std::nullopt);

[[nodiscard]] std::vector<SliceCatalogEntry> LoadSliceCatalog(
