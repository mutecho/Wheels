...
void readCF_generic(TFile* filein, int evtType, 
                    int phiBin, bool usePhi,
...
void projectCF1D(TH1F*& hCF1D, TH1F*& hCF1DBkg, TH3F* hNum3D, TH3F* hDen3D, TString target, TF1*& fNormBkg, int qn, int binsFromZero = 1);
TF1* normBkg(TH1F*& hfCF1D, int iqn);
float norm(TH1F*& hfCF1D);
void drawAndCheck(TString drawOpt, vector<int> color, int canvaswidth, int canvasHight, 
                  bool isDivided, bool isMultPlots, int divideX, int divideY, 
                  TH1F* hDrawSingle, vector<TH1F*> hDraw, vector<TF1*> fBkg={});
	...
	switch (analysisMode)
	{
		case 0:{ // draw and test correlation functions
					...
					for (int iqn=0; iqn<nQnBins; iqn++){
						projectCF1D(histoCF_SM[0][0][iqn], histoCF_bkg[0][0], histoNum[0][0][iqn], histoDen[0][0], "out", fnormbkg[0][0][iqn], iqn);
					}
					...
					drawAndCheck("pl", {kRe, kBl, kGr}, 
								 900, 600, false, true, -1, -1, nullptr, outPlots);
		...
		case 1:{ 
					...
					// resize storage
					histoNum_phi.clear();
        			histoNum_phi.resize(nPhiBins, vector<vector<vector<TH3F*>>>(nMtBins, vector<vector<TH3F*>>(nCentBins, vector<TH3F*>(nQnBins,nullptr))));
        ...
        for(int iCent=0;iCent<nCentBins;iCent++){
            ...
            if(evtType==kMIXED){
            	...
            	for(int iqn=0;iqn<nQnBins;iqn++){
            	    ...
            	    if(evtType==kSAME){
            	        if(iqn==0) sp->GetAxis(kQn)->SetRange(1,10);
...
int GetRootColor(int c){
  if (c == kRe)  return kRed-4;
  if (c == kBl)  return kAzure+1;
  if (c == kGr)  return kGreen-2;
  if (c == kPk)  return kPink+8;
  if (c == kPur) return kViolet-1;
  if (c == kYl)  return kYellow-3;
  if (c == kGry) return kGray+1;
  if (c == kBlk) return kBlack;
  return kBlack;
}
