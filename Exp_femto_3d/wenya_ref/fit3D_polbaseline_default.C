...
void fit3D_polbaseline(int mod = 0) {
  ...
  histoSE.resize(
      nCentBins,
      vector<vector<vector<TH3F *>>>(nMtBins, vector<vector<TH3F *>>(nQnBins, vector<TH3F *>(nPhiBins, nullptr))));
    ...
    for (int iMt = 0; iMt < nMtBins; iMt++) {
      for (int iqn = 0; iqn < nQnBins; iqn++) {
        for (int iPhi = 5; iPhi < nPhiBins; iPhi++) {
          histoSE[iCent][iMt][iqn][iPhi] =
              (TH3F *)file->Get(Form("H3_same_cent%d_mt%d_qn%d_phi%d", iCent, iMt, iqn, iPhi));
        }
      }
    }
  }
  ...
  for (int iCent = 2; iCent < nCentBins; iCent++) {
    for (int iMt = 0; iMt < nMtBins; iMt++) {
      for (int iqn = 0; iqn < nQnBins; iqn++) {
        for (int iPhi = 5; iPhi < nPhiBins; iPhi++) {
          Fit3D_BowlerSinyukov_Full((int)centBin[iCent],
                                    (double)mTBin[iMt],
                                    histoSE[iCent][iMt][iqn][iPhi],
                                    histoME[iCent][iMt][iPhi],
                                    iqn,
                                    iPhi,
                                    lam[iCent][iMt][iqn][iPhi],
                                    Ro2[iCent][iMt][iqn][iPhi],
                                    Rs2[iCent][iMt][iqn][iPhi],
                                    Rl2[iCent][iMt][iqn][iPhi],
                                    lamerr[iCent][iMt][iqn][iPhi],
                                    Ro2err[iCent][iMt][iqn][iPhi],
                                    Rs2err[iCent][iMt][iqn][iPhi],
                                    Rl2err[iCent][iMt][iqn][iPhi],
                                    n0[iCent][iMt][iqn][iPhi],
                                    n1[iCent][iMt][iqn][iPhi],
                                    n0err[iCent][iMt][iqn][iPhi],
                                    n1err[iCent][iMt][iqn][iPhi],
                                    mod);
        }
      }
    }
  ...
  // for (int iMt=0; iMt<nMtBins; iMt++){
  // for (int iqn=0; iqn<nQnBins; iqn++){
  // 	histoRoutVsPhi[iCent][iMt][iqn] = new TH1F(Form("ro2_cent%d_mt%d_qn%d",iCent,iMt,iqn),";#phi^{pair} -
  // #Psi_EP;R_{out}^{2}", 12, -0.2, 3.4); 	for (int iPhi=0; iPhi<nPhiBins; iPhi++){
  ...
  // for (int iMt=0; iMt<nMtBins; iMt++){
  // for (int iqn=0; iqn<nQnBins; iqn++){
  // 	histoRsideVsPhi[iCent][iMt][iqn] = new TH1F(Form("rs2_cent%d_mt%d_qn%d",iCent,iMt,iqn),";#phi^{pair} -
  // #Psi_EP;R_{out}^{2}", 12, -0.2, 3.4); 	for (int iPhi=0; iPhi<nPhiBins; iPhi++){
  ...
  // for (int iMt=0; iMt<nMtBins; iMt++){
  // for (int iqn=0; iqn<nQnBins; iqn++){
  // 	histoRlongVsPhi[iCent][iMt][iqn] = new TH1F(Form("rl2_cent%d_mt%d_qn%d",iCent,iMt,iqn),";#phi^{pair} -
  // #Psi_EP;R_{out}^{2}", 12, -0.2, 3.4); 	for (int iPhi=0; iPhi<nPhiBins; iPhi++){
  ...
  // for (int iMt=0; iMt<nMtBins; iMt++){
  // for (int iqn=0; iqn<nQnBins; iqn++){
  // 	histolamVsPhi[iCent][iMt][iqn] = new TH1F(Form("lam_cent%d_mt%d_qn%d",iCent,iMt,iqn),";#phi^{pair} -
  // #Psi_EP;R_{out}^{2}", 12, -0.2, 3.4); 	for (int iPhi=0; iPhi<nPhiBins; iPhi++){
  ...
  // for (int iMt=0; iMt<nMtBins; iMt++){
  // for (int iqn=0; iqn<nQnBins; iqn++){
  // 	histoN0VsPhi[iCent][iMt][iqn] = new TH1F(Form("n0_cent%d_mt%d_qn%d",iCent,iMt,iqn),";#phi^{pair} -
  // #Psi_EP;R_{out}^{2}", 12, -0.2, 3.4); 	for (int iPhi=0; iPhi<nPhiBins; iPhi++){
  ...
  // for (int iMt=0; iMt<nMtBins; iMt++){
  // for (int iqn=0; iqn<nQnBins; iqn++){
  // 	histoN1VsPhi[iCent][iMt][iqn] = new TH1F(Form("n1_cent%d_mt%d_qn%d",iCent,iMt,iqn),";#phi^{pair} -
  // #Psi_EP;R_{out}^{2}", 12, -0.2, 3.4); 	for (int iPhi=0; iPhi<nPhiBins; iPhi++){
