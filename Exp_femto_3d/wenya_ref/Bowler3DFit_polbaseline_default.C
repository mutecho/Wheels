...
static TSpline3 *GetCoulombSpline(int cent) {
  ...
  if (itS != gCoulSpl.end() && itS->second)
    return itS->second;

  const char *path = nullptr;
  if (cent == 0)
    path = "./CRAB_version3/cf_kc_cent0to10.root";
  if (cent == 10)
    path = "./CRAB_version3/cf_kc_cent10to30.root";
  if (cent == 30)
    path = "./CRAB_version3/cf_kc_cent30to50.root";
  if (cent == 50)
    path = "./CRAB_version3/cf_kc_cent50to80.root";

  // 打开/复用文件
  TFile *f = nullptr;
  auto itF = gCoulFile.find(cent);
  if (itF != gCoulFile.end())
    f = itF->second;
...
//   8: Rsl2  = R_sl^2
//--------------------------------------------------------
double CF_BowlerSinyukov_3D(double qout, double qside, double qlong, const double *par) {
  ...
  double qinvLow = TMath::Sqrt((fabs(qout) - 0.01) * (fabs(qout) - 0.01) + (fabs(qside) - 0.01) * (fabs(qside) - 0.01)
                               + (fabs(qlong) - 0.01) * (fabs(qlong) - 0.01));
  double qinvUp = TMath::Sqrt((fabs(qout) + 0.01) * (fabs(qout) + 0.01) + (fabs(qside) + 0.01) * (fabs(qside) + 0.01)
                              + (fabs(qlong) + 0.01) * (fabs(qlong) + 0.01));
  ...
  double exponent = -(Ro2 * qout * qout * factor + Rs2 * qside * qside * factor + Rl2 * qlong * qlong * factor
                      + 2.0 * Ros2 * qout * qside * factor + 2.0 * Rol2 * qout * qlong * factor
...
double fSide_1D(double *x, double *) {
  double qside = x[0];
  return (CF_BowlerSinyukov_3D(-0.011, qside, 0.0, gBestParForSlice)
...
double fLong_1D(double *x, double *) {
  double qlong = x[0];
  return (CF_BowlerSinyukov_3D(-0.011, 0.0, qlong, gBestParForSlice)
...
//--------------------------------------------------------
void Fit3D_BowlerSinyukov_Full(int cent,
                               double mT,
                               TH3F *hNum,
                               TH3F *hDen,
                               int iqn,
  ...
  if (cent == 0) {
    if (mT == 0.2)
      hKStar = (TH1F *)fk->Get(Form("hkstar_same_cent0_mt0"));
    if (mT == 0.3)
      hKStar = (TH1F *)fk->Get(Form("hkstar_same_cent0_mt1"));
    if (mT == 0.5)
      hKStar = (TH1F *)fk->Get(Form("hkstar_same_cent0_mt2"));
  }
  if (cent == 10) {
    if (mT == 0.2)
      hKStar = (TH1F *)fk->Get(Form("hkstar_same_cent1_mt0"));
    if (mT == 0.3)
      hKStar = (TH1F *)fk->Get(Form("hkstar_same_cent1_mt1"));
    if (mT == 0.5)
      hKStar = (TH1F *)fk->Get(Form("hkstar_same_cent1_mt2"));
  }
  if (cent == 30) {
    if (mT == 0.2)
      hKStar = (TH1F *)fk->Get(Form("hkstar_same_cent2_mt0"));
    if (mT == 0.3)
      hKStar = (TH1F *)fk->Get(Form("hkstar_same_cent2_mt1"));
    if (mT == 0.5)
      hKStar = (TH1F *)fk->Get(Form("hkstar_same_cent2_mt2"));
  }
  if (cent == 50) {
    if (mT == 0.2)
      hKStar = (TH1F *)fk->Get(Form("hkstar_same_cent3_mt0"));
    if (mT == 0.3)
      hKStar = (TH1F *)fk->Get(Form("hkstar_same_cent3_mt1"));
    if (mT == 0.5)
      hKStar = (TH1F *)fk->Get(Form("hkstar_same_cent3_mt2"));
  }
  ...
  if (mod == 0) {
    ...
    std::cout << "Ro2    = " << val << "  ±  " << err << "   => R_out = " << TMath::Sqrt(TMath::Max(val, 0.0)) << "\n";

    minuit.GetParameter(startSourcePara + 2, val, err);
    std::cout << "Rs2    = " << val << "  ±  " << err << "   => R_side = " << TMath::Sqrt(TMath::Max(val, 0.0)) << "\n";

    minuit.GetParameter(startSourcePara + 3, val, err);
    std::cout << "Rl2    = " << val << "  ±  " << err << "   => R_long = " << TMath::Sqrt(TMath::Max(val, 0.0)) << "\n";
