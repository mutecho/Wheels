...
ApplicationConfig LoadApplicationConfig(const std::string& path) {
  ...
  config.input.same_event_subtask =
      ReadRequiredString(input, "same_event_subtask", "input");
  config.input.mixed_event_subtask =
      ReadRequiredString(input, "mixed_event_subtask", "input");
  config.input.sparse_object_name =
      ReadRequiredString(input, "sparse_object_name", "input");

  config.output.output_directory =
      ReadRequiredString(output, "output_directory", "output");
  config.output.cf_root_name =
      ReadOptionalString(output, "cf_root_name", config.output.cf_root_name);
  config.output.fit_root_name =
      ReadOptionalString(output, "fit_root_name", config.output.fit_root_name);
  config.output.fit_summary_name = ReadOptionalString(
      output, "fit_summary_name", config.output.fit_summary_name);
  config.output.log_level = ParseLogLevel(
      ReadOptionalString(output, "log_level", ToString(config.output.log_level)));

  config.build.map_pair_phi_to_symmetric_range = ReadOptionalBool(
      build, "map_pair_phi_to_symmetric_range",
      config.build.map_pair_phi_to_symmetric_range);
  config.build.write_normalized_se_me_1d_projections = ReadOptionalBool(
      build, "write_normalized_se_me_1d_projections",
      config.build.write_normalized_se_me_1d_projections);
  config.build.reopen_output_file_per_slice = ReadOptionalBool(
      build, "reopen_output_file_per_slice",
      config.build.reopen_output_file_per_slice);

  config.fit.model = ParseFitModel(
      ReadOptionalString(fit, "model", ToString(config.fit.model)));
  config.fit.options.use_coulomb =
      ReadOptionalBool(fit, "use_coulomb", config.fit.options.use_coulomb);
  config.fit.options.use_core_halo_lambda = ReadOptionalBool(
      fit, "use_core_halo_lambda", config.fit.options.use_core_halo_lambda);
  config.fit.options.use_q2_baseline =
      ReadOptionalBool(fit, "use_q2_baseline", config.fit.options.use_q2_baseline);
  config.fit.options.use_pml =
      ReadOptionalBool(fit, "use_pml", config.fit.options.use_pml);
  config.fit.options.fit_q_max =
      ReadOptionalDouble(fit, "fit_q_max", config.fit.options.fit_q_max);
  config.fit.reopen_output_file_per_slice = ReadOptionalBool(
      fit, "reopen_output_file_per_slice", config.fit.reopen_output_file_per_slice);

  if (const auto* bins = root["bins"].as_table(); bins != nullptr) {
    config.centrality_bins =
        ParseRangeBinArray(GetOptionalArray(*bins, "centrality"), "bins.centrality");
    config.mt_bins = ParseRangeBinArray(GetOptionalArray(*bins, "mt"), "bins.mt");
  }

  if (const auto* fit_selection = root["fit_selection"].as_table();
      fit_selection != nullptr) {
    config.fit_centrality_bins = ParseRangeBinArray(
        GetOptionalArray(*fit_selection, "centrality"), "fit_selection.centrality");
    config.fit_mt_bins =
        ParseRangeBinArray(GetOptionalArray(*fit_selection, "mt"), "fit_selection.mt");
  }
...
void ValidateApplicationConfig(ApplicationConfig& config) {
  ...
  if (config.fit.options.fit_q_max <= 0.0 ||
      !std::isfinite(config.fit.options.fit_q_max)) {
  ...
  for (const RangeBin& bin : config.fit_centrality_bins) {
    if (!ContainsExactRange(config.centrality_bins, bin)) {
      throw ConfigError(
          "Each fit_selection.centrality bin must exactly match a build bin.");
    }
  }
