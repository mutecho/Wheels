...
std::string BuildGroupId(const RangeBin& centrality_bin, const RangeBin& mt_bin) {
  return "cent_" + SanitizeToken(FormatDouble(centrality_bin.min, 2)) + "_" +
         SanitizeToken(FormatDouble(centrality_bin.max, 2)) + "__mt_" +
         SanitizeToken(FormatDouble(mt_bin.min, 2)) + "_" +
         SanitizeToken(FormatDouble(mt_bin.max, 2));
}
...
std::unique_ptr<TFile> OpenRootFile(const std::string& path, const char* mode) {
  ...
  if (!file || file->IsZombie()) {
    throw std::runtime_error("Cannot open ROOT file: " + path + " with mode " +
                             mode);
  }
  return file;
}
...
TDirectory* GetOrCreateDirectoryPath(TDirectory& parent, const std::string& path) {
  ...
  while (begin < path.size()) {
    ...
    const std::string component =
        path.substr(begin, separator == std::string::npos ? std::string::npos
                                                          : separator - begin);
    if (!component.empty()) {
...
double IntegralVisibleRange(TH3D* hist, const bool use_width = false) {
  ...
  return hist->Integral(1, hist->GetNbinsX(), 1, hist->GetNbinsY(), 1,
                        hist->GetNbinsZ(), use_width ? "width" : "");
}
...
TH1D* BuildProjectionXWithinWindow(TH3D* hist, const std::string& name, const double q_max) {
  ...
  const int n_window_bins =
      std::max(0, y_max - y_min + 1) * std::max(0, z_max - z_min + 1);
  if (n_window_bins > 0) {
...
TH1D* BuildProjectionYWithinWindow(TH3D* hist, const std::string& name, const double q_max) {
  ...
  const int n_window_bins =
      std::max(0, x_max - x_min + 1) * std::max(0, z_max - z_min + 1);
  if (n_window_bins > 0) {
    projection->Scale(1.0 / static_cast<double>(n_window_bins));
  }
  return projection;
...
TH1D* BuildProjectionZWithinWindow(TH3D* hist, const std::string& name, const double q_max) {
  ...
  const int n_window_bins =
      std::max(0, x_max - x_min + 1) * std::max(0, y_max - y_min + 1);
  if (n_window_bins > 0) {
    projection->Scale(1.0 / static_cast<double>(n_window_bins));
  }
  return projection;
...
void Write1DProjections(TH3D* histogram,
  ...
  if (use_window) {
    projection_x =
        BuildProjectionXWithinWindow(histogram, base_name + "_ProjX", kProjection1DWindow);
    projection_y =
        BuildProjectionYWithinWindow(histogram, base_name + "_ProjY", kProjection1DWindow);
    projection_z =
        BuildProjectionZWithinWindow(histogram, base_name + "_ProjZ", kProjection1DWindow);
  } else {
    projection_x = histogram->ProjectionX((base_name + "_ProjX").c_str(), 1,
                                          histogram->GetNbinsY(), 1,
                                          histogram->GetNbinsZ());
    projection_y = histogram->ProjectionY((base_name + "_ProjY").c_str(), 1,
                                          histogram->GetNbinsX(), 1,
                                          histogram->GetNbinsZ());
    projection_z = histogram->ProjectionZ((base_name + "_ProjZ").c_str(), 1,
                                          histogram->GetNbinsX(), 1,
                                          histogram->GetNbinsY());
  }
...
TH3D* LoadStoredHistogram3D(TFile& input_file,
...
std::string BuildSparseObjectPath(const ApplicationConfig& config,
                                  const std::string& subtask_name) {
  return config.input.task_name + "/" + subtask_name + "/" +
         config.input.sparse_object_name;
}
...
bool MatchSelectedBin(const SliceCatalogEntry& entry,
  ...
  const bool centrality_matched =
      std::any_of(centrality_bins.begin(), centrality_bins.end(),
                  [&](const RangeBin& bin) {
                    return NearlyEqual(entry.cent_low, bin.min) &&
                           NearlyEqual(entry.cent_high, bin.max);
                  });
...
double ComputeLikeSignPiPiGamowFactor(const double q_out,
                                      const double q_side,
                                      const double q_long) {
  const double q_magnitude = std::sqrt(q_out * q_out + q_side * q_side +
                                       q_long * q_long);
...
double ComputeBowlerSinyukovLikeSignPiPiValue(const double norm,
  ...
  const double coulomb_factor =
      fit_options.use_coulomb
          ? ComputeLikeSignPiPiGamowFactor(q_out, q_side, q_long)
          : 1.0;
  const double quantum_stat_term = std::exp(-levy_exponent);
  return norm * ((1.0 - lambda_eff) +
                 lambda_eff * coulomb_factor * (1.0 + quantum_stat_term));
}
...
bool IsFullR2MatrixPositiveSemiDefinite(const double rout2,
  ...
  if (det_out_side < -tolerance || det_out_long < -tolerance ||
      det_side_long < -tolerance) {
    return false;
  }
...
bool HasValidFullR2MatrixFromParameterArray(const double* parameters) {
  ...
  return IsFullR2MatrixPositiveSemiDefinite(parameters[2], parameters[3], parameters[4],
...
double EvaluateDiagonalLevyCF(const double q_out,
  ...
  const double argument =
      (rout2 * q_out2 + rside2 * q_side2 + rlong2 * q_long2) / (kHbarC * kHbarC);
  const double levy_exponent = std::pow(std::max(argument, 0.0), alpha / 2.0);
  const double femto_value = ComputeBowlerSinyukovLikeSignPiPiValue(
      norm, lambda, levy_exponent, fit_options, q_out, q_side, q_long);
  return femto_value *
         ComputeQ2BaselineFactor(q_out, q_side, q_long, baseline_q2, fit_options);
}
...
double EvaluateFullLevyCF(const double q_out,
  ...
  const double argument =
      (rout2 * q_out * q_out + rside2 * q_side * q_side + rlong2 * q_long * q_long +
       2.0 * routside2 * q_out * q_side + 2.0 * routlong2 * q_out * q_long +
       2.0 * rsidelong2 * q_side * q_long) /
      (kHbarC * kHbarC);
...
double Levy3DModel(double* x, double* parameters) {
  ...
  return EvaluateDiagonalLevyCF(x[0], x[1], x[2], parameters[0], parameters[1],
                                parameters[2], parameters[3], parameters[4],
                                parameters[5], parameters[6], fit_options);
}
...
double Levy3DFullModel(double* x, double* parameters) {
  ...
  return EvaluateFullLevyCF(x[0], x[1], x[2], parameters[0], parameters[1],
                            parameters[2], parameters[3], parameters[4],
                            parameters[5], parameters[6], parameters[7],
                            parameters[8], parameters[9], fit_options);
}

TF3* BuildLevyFitFunction(const std::string& function_name,
                          const LevyFitOptions& fit_options) {
  ...
  auto* fit_function =
      new TF3(function_name.c_str(), Levy3DModel, -fit_options.fit_q_max,
              fit_options.fit_q_max, -fit_options.fit_q_max, fit_options.fit_q_max,
              -fit_options.fit_q_max, fit_options.fit_q_max, 10);
...
TF3* BuildFullLevyFitFunction(const std::string& function_name,
                              const LevyFitOptions& fit_options) {
  const double q2_max = 3.0 * fit_options.fit_q_max * fit_options.fit_q_max;
  const double baseline_min = q2_max > 0.0 ? -0.9 / q2_max : -10.0;
  const double baseline_max = q2_max > 0.0 ? 2.0 / q2_max : 10.0;
  auto* fit_function =
      new TF3(function_name.c_str(), Levy3DFullModel, -fit_options.fit_q_max,
              fit_options.fit_q_max, -fit_options.fit_q_max, fit_options.fit_q_max,
              -fit_options.fit_q_max, fit_options.fit_q_max, 13);
  fit_function->SetParName(0, "Norm");
  fit_function->SetParName(1, "lambda");
  fit_function->SetParName(2, "Rout2");
  fit_function->SetParName(3, "Rside2");
  ...
  const double initial_parameters[13] = {1.0, 0.5, 25.0, 25.0, 25.0, 0.0, 0.0,
...
double ComputePMLNeg2LogLContribution(const double same_counts,
  ...
  if (same_counts < 0.0 || mixed_counts < 0.0 || model_ratio <= 0.0 ||
      !std::isfinite(model_ratio)) {
  ...
  if (mixed_counts == 0.0) {
    const double log_term =
        model_ratio >= 1.0 ? std::log1p(1.0 / model_ratio)
                           : std::log1p(model_ratio) - std::log(model_ratio);
    return 2.0 * same_counts * log_term;
  }

  const double total_counts = same_counts + mixed_counts;
  const double arg1 =
      model_ratio * total_counts / (same_counts * (model_ratio + 1.0));
...
void Levy3DPMLFCN(Int_t& npar, Double_t* grad, Double_t& f, Double_t* parameters,
                  Int_t flag) {
  (void)npar;
  (void)grad;
  (void)flag;
  if (g_levy_3d_pml_context.h_se_raw == nullptr ||
      g_levy_3d_pml_context.h_me_raw == nullptr) {
    f = kFitPenaltyValue;
    return;
  }
  if (g_levy_3d_pml_context.use_full_model &&
      !HasValidFullR2MatrixFromParameterArray(parameters)) {
    f = kFitPenaltyValue;
    return;
  }
  ...
  for (int ix = 1; ix <= g_levy_3d_pml_context.h_se_raw->GetNbinsX(); ++ix) {
    ...
    for (int iy = 1; iy <= g_levy_3d_pml_context.h_se_raw->GetNbinsY(); ++iy) {
      const double q_side =
          g_levy_3d_pml_context.h_se_raw->GetYaxis()->GetBinCenter(iy);
      if (std::abs(q_side) > g_levy_3d_pml_context.fit_options.fit_q_max) {
        continue;
      }
      for (int iz = 1; iz <= g_levy_3d_pml_context.h_se_raw->GetNbinsZ(); ++iz) {
        const double q_long =
            g_levy_3d_pml_context.h_se_raw->GetZaxis()->GetBinCenter(iz);
        if (std::abs(q_long) > g_levy_3d_pml_context.fit_options.fit_q_max) {
          continue;
        }

        const double same_counts =
            g_levy_3d_pml_context.h_se_raw->GetBinContent(ix, iy, iz);
        const double mixed_counts =
            g_levy_3d_pml_context.h_me_raw->GetBinContent(ix, iy, iz);
        if (same_counts == 0.0 && mixed_counts == 0.0) {
          continue;
        }
        ...
        const double contribution =
            ComputePMLNeg2LogLContribution(same_counts, mixed_counts, model_ratio);
  ...
  for (int ix = 1; ix <= h_se_raw->GetNbinsX(); ++ix) {
    ...
    for (int iy = 1; iy <= h_se_raw->GetNbinsY(); ++iy) {
      ...
      for (int iz = 1; iz <= h_se_raw->GetNbinsZ(); ++iz) {
        ...
        if (h_se_raw->GetBinContent(ix, iy, iz) +
                h_me_raw->GetBinContent(ix, iy, iz) >
            0.0) {
          ++n_points;
        }
      }
...
bool IsPMLParameterFixed(const int parameter_index,
                         const bool use_full_model,
                         const LevyFitOptions& fit_options) {
  if (!use_full_model) {
...
void ConfigurePMLMinuit(TMinuit& minuit,
  ...
  for (int index = 0; index < n_parameters; ++index) {
    ...
    if (fixed) {
      minuit.mnparm(index, fit_function->GetParName(index), value, step, 0.0, 0.0,
                    error_code);
      minuit.FixParameter(index);
    } else {
      minuit.mnparm(index, fit_function->GetParName(index), value, step, lower, upper,
                    error_code);
    }
  }
}
...
bool RunPMLFit(TF3* fit_function,
  ...
  fit_statistic = fmin;
  edm = fedm;
  ndf = std::max(0, CountPMLUsableBins(h_se_raw, h_me_raw, fit_options.fit_q_max) -
                        npari);
  minuit_istat = istat;
...
TGraph* BuildProjectionCurveXWithinWindow(TF3* fit_function,
  ...
  const double x_max = reference_histogram->GetXaxis()->GetBinUpEdge(
      reference_histogram->GetNbinsX());
  for (int index = 0; index < n_samples; ++index) {
    const double x = x_min + (x_max - x_min) * static_cast<double>(index) /
                               static_cast<double>(n_samples - 1);
    graph->SetPoint(index, x,
                    EvaluateProjectionXWindowAverage(fit_function, reference_histogram,
                                                     x, q_max));
  }
  return graph;
}
...
TGraph* BuildProjectionCurveYWithinWindow(TF3* fit_function,
  ...
  const double x_max = reference_histogram->GetYaxis()->GetBinUpEdge(
      reference_histogram->GetNbinsY());
  for (int index = 0; index < n_samples; ++index) {
    const double x = x_min + (x_max - x_min) * static_cast<double>(index) /
                               static_cast<double>(n_samples - 1);
    graph->SetPoint(index, x,
                    EvaluateProjectionYWindowAverage(fit_function, reference_histogram,
                                                     x, q_max));
  }
  return graph;
}
...
TGraph* BuildProjectionCurveZWithinWindow(TF3* fit_function,
  ...
  const double x_max = reference_histogram->GetZaxis()->GetBinUpEdge(
      reference_histogram->GetNbinsZ());
  for (int index = 0; index < n_samples; ++index) {
    const double x = x_min + (x_max - x_min) * static_cast<double>(index) /
                               static_cast<double>(n_samples - 1);
    graph->SetPoint(index, x,
                    EvaluateProjectionZWindowAverage(fit_function, reference_histogram,
                                                     x, q_max));
  }
  return graph;
}
...
std::string FormatParameterLine(const std::string& label,
  ...
  std::ostringstream stream;
  stream << std::fixed << std::setprecision(3) << label << " = " << value << " #pm "
         << error;
  if (!unit.empty()) {
    stream << " " << unit;
  }
...
std::string BuildFitSwitchLine(const LevyFitResult& fit_result) {
  return std::string("Coulomb(#pi^{#pm}#pi^{#pm}): ") +
         (fit_result.uses_coulomb ? "on" : "off") +
...
std::string BuildFitStatisticLine(const LevyFitResult& fit_result) {
  ...
  stream << std::fixed << std::setprecision(2)
         << (fit_result.uses_pml ? "-2 ln L/NDF = " : "#chi^{2}/NDF = ")
         << fit_result.fit_statistic << "/" << fit_result.ndf;
  return stream.str();
}

TPaveText* BuildFitParameterBox(const LevyFitResult& fit_result,
                                const double x1,
                                const double y1,
                                const double x2,
                                const double y2) {
  ...
  if (fit_result.uses_core_halo_lambda) {
    box->AddText(
        FormatParameterLine("#lambda", fit_result.lambda, fit_result.lambda_err).c_str());
  } else {
  ...
  if (fit_result.uses_q2_baseline) {
    box->AddText(FormatParameterLine("b_{q^{2}}", fit_result.baseline_q2,
                                     fit_result.baseline_q2_err, "(GeV/c)^{-2}")
                     .c_str());
  } else {
    box->AddText("b_{q^{2}} fixed = 0.000");
  }
  box->AddText(
      FormatParameterLine("R_{out}^{2}", fit_result.rout2, fit_result.rout2_err, "fm^{2}")
          .c_str());
  box->AddText(FormatParameterLine("R_{side}^{2}", fit_result.rside2,
                                   fit_result.rside2_err, "fm^{2}")
                   .c_str());
  box->AddText(FormatParameterLine("R_{long}^{2}", fit_result.rlong2,
                                   fit_result.rlong2_err, "fm^{2}")
                   .c_str());
  if (fit_result.has_off_diagonal) {
    box->AddText(FormatParameterLine("R_{outside}^{2}", fit_result.routside2,
                                     fit_result.routside2_err, "fm^{2}")
                     .c_str());
    box->AddText(FormatParameterLine("R_{outlong}^{2}", fit_result.routlong2,
                                     fit_result.routlong2_err, "fm^{2}")
                     .c_str());
    box->AddText(FormatParameterLine("R_{sidelong}^{2}", fit_result.rsidelong2,
                                     fit_result.rsidelong2_err, "fm^{2}")
                     .c_str());
  }
  box->AddText((std::string("Fit method: ") + (fit_result.uses_pml ? "PML" : "chi2"))
                   .c_str());
  ...
  if (fit_result.uses_pml) {
    ...
    box->AddText((std::string("istat = ") + std::to_string(fit_result.minuit_istat))
                     .c_str());
    box->AddText((std::string("Cov quality: ") +
                  DescribeCovarianceQuality(fit_result.minuit_istat))
                     .c_str());
  }
  return box;
}
...
TCanvas* BuildProjectionCanvas(const std::string& canvas_name,
  ...
  const double max_value =
      std::max(data_histogram->GetMaximum(), GetGraphMaximum(fit_graph));
  ...
  const double y1 = fit_result.has_off_diagonal
                        ? (fit_result.uses_pml ? 0.32 : 0.40)
                        : (fit_result.uses_pml ? 0.42 : 0.50);
...
TCanvas* Build3DComparisonCanvas(const std::string& canvas_name,
  ...
  const double y1 = fit_result.has_off_diagonal
                        ? (fit_result.uses_pml ? 0.18 : 0.28)
                        : (fit_result.uses_pml ? 0.30 : 0.40);
...
void FillFitResultFromFunction(const SliceCatalogEntry& entry,
  ...
  fit_result.phi = entry.is_phi_integrated ? std::numeric_limits<double>::quiet_NaN()
                                           : entry.display_phi_center;
  ...
  fit_result.lambda =
      fit_options.use_core_halo_lambda ? fit_function->GetParameter(1) : 1.0;
  fit_result.lambda_err =
      fit_options.use_core_halo_lambda ? fit_function->GetParError(1) : 0.0;
  ...
  if (model == FitModel::kFull) {
    ...
    fit_result.baseline_q2 =
        fit_options.use_q2_baseline ? fit_function->GetParameter(9) : 0.0;
    fit_result.baseline_q2_err =
        fit_options.use_q2_baseline ? fit_function->GetParError(9) : 0.0;
  } else {
    fit_result.alpha = fit_function->GetParameter(5);
    fit_result.alpha_err = fit_function->GetParError(5);
    fit_result.baseline_q2 =
        fit_options.use_q2_baseline ? fit_function->GetParameter(6) : 0.0;
    fit_result.baseline_q2_err =
        fit_options.use_q2_baseline ? fit_function->GetParError(6) : 0.0;
  }
...
bool FitAndWriteSingleSlice(TH3D* h_cf,
  ...
  if (fit_options.use_pml) {
    fit_succeeded = RunPMLFit(fit_function, h_se_raw, h_me_raw, model == FitModel::kFull,
                              fit_options, fit_statistic, fit_ndf, fit_status, fit_edm,
                              fit_minuit_istat);
  } else {
  ...
  FillFitResultFromFunction(entry, fit_function, model, fit_options, fit_statistic,
                            fit_ndf, fit_status, fit_edm, fit_minuit_istat, fit_result);
  if (!fit_succeeded) {
    delete fit_function;
    return false;
  }
  ...
  auto* projection_x_data =
      BuildProjectionXWithinWindow(h_cf, entry.slice_id + "_data_ProjX", kProjection1DWindow);
  auto* projection_y_data =
      BuildProjectionYWithinWindow(h_cf, entry.slice_id + "_data_ProjY", kProjection1DWindow);
  auto* projection_z_data =
      BuildProjectionZWithinWindow(h_cf, entry.slice_id + "_data_ProjZ", kProjection1DWindow);

  auto* projection_x_fit = BuildProjectionCurveXWithinWindow(
      fit_function, h_cf, entry.slice_id + "_fit_ProjX", kProjection1DWindow);
  auto* projection_y_fit = BuildProjectionCurveYWithinWindow(
      fit_function, h_cf, entry.slice_id + "_fit_ProjY", kProjection1DWindow);
  auto* projection_z_fit = BuildProjectionCurveZWithinWindow(
      fit_function, h_cf, entry.slice_id + "_fit_ProjZ", kProjection1DWindow);

  auto* canvas_x = BuildProjectionCanvas(entry.slice_id + "_canvas_ProjX",
                                         projection_x_data, projection_x_fit,
                                         "q_{out} (GeV/c)", fit_result);
  auto* canvas_y = BuildProjectionCanvas(entry.slice_id + "_canvas_ProjY",
                                         projection_y_data, projection_y_fit,
                                         "q_{side} (GeV/c)", fit_result);
  auto* canvas_z = BuildProjectionCanvas(entry.slice_id + "_canvas_ProjZ",
                                         projection_z_data, projection_z_fit,
                                         "q_{long} (GeV/c)", fit_result);
  auto* comparison_canvas = Build3DComparisonCanvas(entry.slice_id + "_canvas_3D",
                                                    h_cf, fit_function, fit_result);
...
void WriteFitResultsSummaryTsv(const std::string& path,
  ...
  for (const LevyFitResult& result : results) {
    output << result.slice_id << "\t" << result.group_id << "\t" << result.fit_model
           << "\t" << (result.uses_coulomb ? 1 : 0) << "\t"
           << (result.uses_core_halo_lambda ? 1 : 0) << "\t"
           << (result.uses_q2_baseline ? 1 : 0) << "\t" << (result.uses_pml ? 1 : 0)
           << "\t" << result.cent_low << "\t" << result.cent_high << "\t"
           << result.mt_low << "\t" << result.mt_high << "\t" << result.phi << "\t"
           << (result.is_phi_integrated ? 1 : 0) << "\t" << result.norm << "\t"
           << result.norm_err << "\t" << result.lambda << "\t" << result.lambda_err
           << "\t" << result.rout2 << "\t" << result.rout2_err << "\t"
           << result.rside2 << "\t" << result.rside2_err << "\t"
           << result.rlong2 << "\t" << result.rlong2_err << "\t"
           << result.routside2 << "\t" << result.routside2_err << "\t"
           << result.routlong2 << "\t" << result.routlong2_err << "\t"
           << result.rsidelong2 << "\t" << result.rsidelong2_err << "\t"
           << result.alpha << "\t" << result.alpha_err << "\t"
           << result.baseline_q2 << "\t" << result.baseline_q2_err << "\t"
           << result.fit_statistic << "\t" << result.edm << "\t" << result.ndf
...
void WriteR2Graphs(TFile& output_file, const std::vector<LevyFitResult>& results) {
  ...
  for (auto& [group_id, group_results] : grouped_results) {
    ...
    std::sort(group_results.begin(), group_results.end(),
              [](const LevyFitResult& lhs, const LevyFitResult& rhs) {
                return lhs.phi < rhs.phi;
              });

    auto* directory =
        GetOrCreateDirectoryPath(output_file, "summary/R2_vs_phi/" + group_id);
    directory->cd();

    const bool uses_mapped_phi_range =
        std::any_of(group_results.begin(), group_results.end(),
                    [](const LevyFitResult& result) { return result.phi < -1.0e-6; });
    ...
    const bool has_off_diagonal = std::any_of(
        group_results.begin(), group_results.end(),
        [](const LevyFitResult& result) { return result.has_off_diagonal; });
    const bool uses_core_halo_lambda = std::any_of(
        group_results.begin(), group_results.end(), [](const LevyFitResult& result) {
          return result.uses_core_halo_lambda;
        });
    const bool uses_q2_baseline = std::any_of(
        group_results.begin(), group_results.end(),
        [](const LevyFitResult& result) { return result.uses_q2_baseline; });
    ...
    TGraphErrors* g_baseline_q2 =
        uses_q2_baseline ? new TGraphErrors(n_points) : nullptr;
    ...
    auto* fit_cos_rout2 = new TF1("Rout2PhiFit", "[0]+2.0*[1]*cos(2.0*x)",
                                  phi_fit_min, phi_fit_max);
    auto* fit_cos_rside2 = new TF1("Rside2PhiFit", "[0]+2.0*[1]*cos(2.0*x)",
                                   phi_fit_min, phi_fit_max);
    auto* fit_cos_rlong2 = new TF1("Rlong2PhiFit", "[0]+2.0*[1]*cos(2.0*x)",
                                   phi_fit_min, phi_fit_max);
    ...
    if (has_off_diagonal) {
      auto* fit_sin_routside2 = new TF1("Routside2PhiFit", "[0]+2.0*[1]*sin(2.0*x)",
                                        phi_fit_min, phi_fit_max);
      auto* fit_cos_routlong2 = new TF1("Routlong2PhiFit", "[0]+2.0*[1]*cos(2.0*x)",
                                        phi_fit_min, phi_fit_max);
      auto* fit_sin_rsidelong2 = new TF1("Rsidelong2PhiFit", "[0]+2.0*[1]*sin(2.0*x)",
                                         phi_fit_min, phi_fit_max);
    ...
    if (uses_q2_baseline) {
      auto* fit_const_baseline =
          new TF1("BaselineQ2PhiFit", "[0]", phi_fit_min, phi_fit_max);
...
BuildCfRunStatistics RunBuildCf(const ApplicationConfig& config, const Logger& logger) {
  ...
  const std::string se_path =
      BuildSparseObjectPath(config, config.input.same_event_subtask);
  const std::string me_path =
      BuildSparseObjectPath(config, config.input.mixed_event_subtask);
  ...
  for (std::size_t centrality_index = 0; centrality_index < config.centrality_bins.size();
       ++centrality_index) {
    ...
    for (std::size_t mt_index = 0; mt_index < config.mt_bins.size(); ++mt_index) {
      ...
      auto write_slice = [&](TH3D* h_se_raw,
        ...
        if (config.build.write_normalized_se_me_1d_projections) {
          Write1DProjections(h_se_norm, *directory, "SE_norm3d", "Normalized density",
                             true);
          Write1DProjections(h_me_norm, *directory, "ME_norm3d", "Normalized density",
                             true);
        }
        output_file->cd();
      ...
      auto* h_se_all_norm =
          static_cast<TH3D*>(h_se_all_raw->Clone((group_id + "_SE_all_norm").c_str()));
      ...
      if (int_se_all == 0.0) {
        ...
        const auto entry = MakeSliceCatalogEntry(
            centrality_bin, mt_bin, static_cast<int>(centrality_index),
            static_cast<int>(mt_index), -1, raw_phi_low, raw_phi_high,
            0.5 * (raw_phi_low + raw_phi_high), raw_phi_low, raw_phi_high,
      ...
      for (int phi_index = 1; phi_index <= n_phi_bins; ++phi_index) {
        ...
        auto* h_se_norm =
            static_cast<TH3D*>(h_se_raw->Clone((group_id + "_SE_slice_norm").c_str()));
        h_se_norm->SetDirectory(nullptr);
        const double int_se = IntegralVisibleRange(h_se_norm, true);
        if (int_se == 0.0) {
          logger.Warn("Zero same-event integral for " + group_id + " phi bin " +
                      std::to_string(phi_index) + "; skipping slice.");
          ++statistics.skipped_zero_same_event_slices;
          delete h_se_raw;
          delete h_se_norm;
          continue;
        ...
        const auto entry = MakeSliceCatalogEntry(
            centrality_bin, mt_bin, static_cast<int>(centrality_index),
            static_cast<int>(mt_index), phi_index - 1, raw_phi_low, raw_phi_high,
            raw_phi_center, display_phi_low, display_phi_high, display_phi_center,
            false);
  ...
  TH1::AddDirectory(old_add_directory);
  logger.Info("Completed build-cf stage: stored " +
              std::to_string(statistics.stored_slices) + " slices.");
  return statistics;
}
...
FitRunStatistics RunFit(const ApplicationConfig& config,
  ...
  const std::string cf_root_path = input_cf_root_path.has_value()
                                       ? ResolvePath(config.output.output_directory,
                                                     *input_cf_root_path)
                                       : ResolvePath(config.output.output_directory,
                                                     config.output.cf_root_name);
  const std::string fit_root_path =
      ResolvePath(config.output.output_directory, config.output.fit_root_name);
  const std::string fit_summary_path =
      ResolvePath(config.output.output_directory, config.output.fit_summary_name);
  ...
  for (const SliceCatalogEntry& entry : catalog_entries) {
    ...
    std::unique_ptr<TH3D> h_cf(
        LoadStoredHistogram3D(*input_file, entry.cf_object_path, entry.slice_id + "_cf_data"));
    if (!h_cf) {
    ...
    if (config.fit.options.use_pml) {
      h_se_raw.reset(
          LoadStoredHistogram3D(*input_file, entry.se_object_path, entry.slice_id + "_se_raw"));
      h_me_raw.reset(
          LoadStoredHistogram3D(*input_file, entry.me_object_path, entry.slice_id + "_me_raw"));
      if (!h_se_raw || !h_me_raw) {
        logger.Warn("PML requested but raw SE/ME histograms are missing for " +
                    entry.slice_id);
    ...
    LevyFitResult fit_result;
    if (FitAndWriteSingleSlice(h_cf.get(), h_se_raw.get(), h_me_raw.get(), entry, model,
                               config.fit.options, fit_root_path, fit_result,
  ...
  TH1::AddDirectory(old_add_directory);
  logger.Info("Completed fit stage: fitted " + std::to_string(statistics.fitted_slices) +
              " slices.");
  return statistics;
}
