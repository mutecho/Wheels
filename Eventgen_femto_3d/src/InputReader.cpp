...
void ValidateScalarLeafType(TTree& tree,
  ...
  if (expected_type != leaf->GetTypeName()) {
    throw std::runtime_error("ROOT branch '" + branch_name + "' has type '" +
                             std::string(leaf->GetTypeName()) +
                             "', expected '" + expected_type + "'.");
  }
}

template <typename ValueType>
void BindScalarBranch(TTree& tree,
                      const std::string& branch_name,
                      ValueType* value) {
...
template <typename ValueType>
void BindVectorBranch(TTree& tree,
                      const std::string& branch_name,
...
std::vector<EventData> LoadLegacyVectorTreeData(
    TFile& input_file,
    const LegacyInputTreeConfig& config) {
  ...
  const bool has_input_event_plane =
      tree.GetBranch(config.event_plane_branch.c_str()) != nullptr;
  ...
  for (Long64_t entry = 0; entry < tree.GetEntries(); ++entry) {
    tree.GetEntry(entry);
    if (pdg == nullptr || px == nullptr || py == nullptr || pz == nullptr ||
        mass == nullptr || x == nullptr || y == nullptr || z == nullptr ||
        t == nullptr) {
      throw std::runtime_error(
          "Legacy vector-tree reader encountered a null particle branch buffer.");
    }
    ...
    for (std::size_t particle_index = 0; particle_index < particle_count;
      ...
      event.particles.push_back(
          MakeParticleState(*pdg, *px, *py, *pz, *mass, *x, *y, *z, *t,
                            particle_index));
    }
...
std::vector<EventData> LoadBlastwaveFlatTreeData(
    TFile& input_file,
    const BlastwaveInputTreeConfig& config) {
  ...
  for (Long64_t entry = 0; entry < events_tree.GetEntries(); ++entry) {
    ...
    if (first_event_id.has_value() && event_id != *previous_event_id + 1) {
      throw std::runtime_error(
          "Blast-wave events tree requires continuous event_id values. Expected " +
          std::to_string(*previous_event_id + 1) + " but found " +
  ...
  for (Long64_t entry = 0; entry < particles_tree.GetEntries(); ++entry) {
    ...
    if (particle_event_id < first_valid_event_id ||
        particle_event_id > last_valid_event_id) {
      throw std::runtime_error(
          "Blast-wave particles tree references unknown event_id=" +
          std::to_string(particle_event_id) + ".");
    }

    const std::size_t event_index =
        static_cast<std::size_t>(particle_event_id - first_valid_event_id);
    ParticleState particle;
    particle.pdg = pid;
    particle.px = px;
    particle.py = py;
...
std::vector<EventData> LoadEventData(const ApplicationConfig& config) {
  std::unique_ptr<TFile> input_file(
      TFile::Open(config.input_root_path.c_str(), "READ"));
  if (input_file == nullptr || input_file->IsZombie()) {
    throw std::runtime_error("Failed to open input ROOT file: " +
  ...
  switch (config.input_schema) {
    ...
    case InputSchema::kBlastwaveFlatTrees:
      return LoadBlastwaveFlatTreeData(*input_file,
