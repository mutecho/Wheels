...
double ComputeSpatialSeparationSquared(const ParticleState& particle1,
...
double ResolveFemtoMtReferenceMass(const ParticleState& particle1,
  ...
  if (std::abs(particle1.mass - particle2.mass) > mass_tolerance) {
    throw std::invalid_argument(
        "Femto mT requires a same-species pair with consistent masses.");
  }
  ...
  if (std::abs(average_mass - configured_reference_mass) > mass_tolerance) {
    throw std::invalid_argument(
        "Configured femto mT reference mass is inconsistent with the selected pair.");
  }
...
PairKinematics ComputePairKinematics(const ParticleState& particle1,
  ...
  const double pair_mt_squared =
      pair.pair_energy * pair.pair_energy - pair.pair_pz * pair.pair_pz;
  if (pair_mt_squared <= kAxisTolerance) {
    throw std::invalid_argument(
        "Pair-system transverse mass is not positive, so the LCMS boost is undefined.");
  }

  const double reference_mass = ResolveFemtoMtReferenceMass(
      particle1, particle2, femto_mt_reference_mass, femto_mt_mass_tolerance);
  pair.pair_system_mt = std::sqrt(pair_mt_squared);
  pair.femto_kt = 0.5 * pair.pair_pt;
  pair.femto_mt =
      std::sqrt(reference_mass * reference_mass + pair.femto_kt * pair.femto_kt);
...
double ComputePairPhiMinusPsi(const PairKinematics& pair_kinematics,
...
PairSeparationLCMS ComputeSinglePairDistanceLCMS(
    const ParticleState& particle1,
    const ParticleState& particle2,
    const double close_pair_distance_tolerance) {
  if (close_pair_distance_tolerance < 0.0) {
    throw std::invalid_argument(
        "Close pair distance tolerance must be non-negative.");
  }

  const double spatial_separation_squared =
      ComputeSpatialSeparationSquared(particle1, particle2);
  if (spatial_separation_squared <
      close_pair_distance_tolerance * close_pair_distance_tolerance) {
    throw std::invalid_argument(
        "Close pair rejection: the spatial separation is too small.");
  }

  const PairKinematics pair = ComputePairKinematics(particle1, particle2);
  if (pair.pair_pt < kAxisTolerance) {
    throw std::invalid_argument(
        "Pair transverse momentum is zero, so the out-side axes are undefined.");
  }
...
PairSeparationLCMS ComputeSinglePairDistanceLCMS(double px1,
  ...
  return ComputeSinglePairDistanceLCMS(particle1,
                                       particle2,
                                       close_pair_distance_tolerance);
}
