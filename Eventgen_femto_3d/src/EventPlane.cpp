...
namespace femto3d {
  ...
  namespace {
    ...
    void ValidateEventPlaneConfig(const EventPlaneConfig &config) {
      ...
      if (config.eta_max <= config.eta_min) {
        throw std::invalid_argument("Event-plane eta maximum must be greater than minimum.");
      }

      if (config.min_q_magnitude < 0.0) {
        throw std::invalid_argument("Event-plane minimum Q-vector magnitude must be non-negative.");
      }
    }

    double ComputeEventPlaneWeight(const ParticleState &particle, const EventPlaneConfig &config) {
  ...
  bool IsEventPlaneCandidate(const ParticleState &particle, const EventPlaneConfig &config) {
    ...
    return std::find(config.allowed_abs_pdg.begin(), config.allowed_abs_pdg.end(), abs_pdg)
           != config.allowed_abs_pdg.end();
  }

  EventPlaneResult ReconstructEventPlane(const std::vector<ParticleState> &event_plane_candidates,
    ...
    result.psi = WrapAngleToEventPlanePeriod(
        std::atan2(result.qy, result.qx) / static_cast<double>(config.harmonic_order), config.harmonic_order);
    result.success = true;
    return result;
  }

  EventPlaneResult MakeInputEventPlaneResult(const double event_plane_psi, const int harmonic_order) {
    EventPlaneResult result;
    result.success = true;
  ...
  double ComputePairPhiMinusEventPlane(const PairKinematics &pair_kinematics,
    ...
    return WrapAngleToEventPlanePeriod(pair_kinematics.pair_phi - event_plane_psi, harmonic_order);
  }

  double ComputePairPhiMinusPsi2(const PairKinematics &pair_kinematics, const double event_plane_psi2) {
