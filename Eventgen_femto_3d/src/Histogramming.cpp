...
namespace femto3d {
  ...
  namespace {
    ...
    void WarnIfOutsideHistogram(const TH3D &histogram, const PairSeparationLCMS &separation) {
      const bool outside_x =
          separation.rho_out < histogram.GetXaxis()->GetXmin() || separation.rho_out >= histogram.GetXaxis()->GetXmax();
      ...
      if (outside_x || outside_y || outside_z) {
        std::cerr << "Warning: value (" << separation.rho_out << ", " << separation.rho_side << ", "
                  << separation.rho_long << ") exceeds TH3D range for histogram " << histogram.GetName() << ".\n";
      }
    }

    void WarnIfOutsideHistogram(const TH1D &histogram, const double value) {
      if (value < histogram.GetXaxis()->GetXmin() || value >= histogram.GetXaxis()->GetXmax()) {
        std::cerr << "Warning: value " << value << " exceeds TH1D range for histogram " << histogram.GetName() << ".\n";
      }
    }
  ...
  void FillToTH3D(TH3D &histogram, const PairSeparationLCMS &separation, const bool warn_on_overflow) {
    if (warn_on_overflow) {
  ...
  std::unique_ptr<TH1D> CreateProjectionHistogram(const std::string &name,
                                                  const AxisSpec &axis,
                                                  const ProjectionDefinition &direction) {
    auto histogram = std::make_unique<TH1D>(name.c_str(), direction.title.c_str(), axis.NumBins(), axis.min, axis.max);
    histogram->SetDirectory(nullptr);
    histogram->Sumw2();
  ...
  SliceHistograms CreateSliceHistograms(const std::string &slice_name,
                                        const AnalysisConfig &config,
                                        const std::array<ProjectionDefinition, 6> &directions) {
    SliceHistograms histograms;
    histograms.source_histogram = CreateTH3D(
        "source_3d", config.histograms.rho_out_axis, config.histograms.rho_side_axis, config.histograms.rho_long_axis);
    histograms.source_histogram->SetTitle(slice_name.c_str());

    for (std::size_t index = 0; index < directions.size(); ++index) {
      const std::string histogram_name = "proj_" + directions[index].short_name;
      histograms.projection_histograms[index] =
          CreateProjectionHistogram(histogram_name, config.histograms.projection_axis, directions[index]);
    }
