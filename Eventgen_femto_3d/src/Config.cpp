...
namespace femto3d {
  ...
  namespace {
    ...
    const toml::table *GetOptionalTable(const toml::table &table, const std::string &key) {
    ...
    int ReadOptionalInt(const toml::table &table, const std::string &key, const int fallback) {
    ...
    std::size_t ReadOptionalSize(const toml::table &table, const std::string &key, const std::size_t fallback) {
      if (const auto value = table[key].value<int64_t>(); value.has_value()) {
        if (*value < 0) {
    ...
    AxisSpec ParseAxisSpec(const toml::table &table, const AxisSpec &fallback, const std::string &context) {
    ...
    bool MatchesRangeBin(const RangeBin &left, const RangeBin &right, const double tolerance = 1.0e-12) {
      return std::abs(left.min - right.min) < tolerance && std::abs(left.max - right.max) < tolerance;
    }
    ...
    void ValidateRangeCollection(const std::string &label, std::vector<RangeBin> &bins) {
      if (bins.empty()) {
    ...
    void ValidateAxis(const std::string &label, const AxisSpec &axis) {
      ...
      if (!std::isfinite(axis.min) || !std::isfinite(axis.max) || axis.max <= axis.min) {
    ...
    void ParseLegacyInputConfig(const toml::table *table, LegacyInputTreeConfig &config) {
      if (table == nullptr) {
        return;
      }
      config.tree_name = ReadOptionalString(*table, "tree_name", config.tree_name);
      config.centrality_branch = ReadOptionalString(*table, "centrality_branch", config.centrality_branch);
      config.event_plane_branch = ReadOptionalString(*table, "event_plane_branch", config.event_plane_branch);
      ...
      config.mass_branch = ReadOptionalString(*table, "mass_branch", config.mass_branch);
    ...
    void ParseBlastwaveInputConfig(const toml::table *table, BlastwaveInputTreeConfig &config) {
      if (table == nullptr) {
        return;
      }
      config.events_tree = ReadOptionalString(*table, "events_tree", config.events_tree);
      config.particles_tree = ReadOptionalString(*table, "particles_tree", config.particles_tree);
      config.event_id_branch = ReadOptionalString(*table, "event_id_branch", config.event_id_branch);
      config.centrality_branch = ReadOptionalString(*table, "centrality_branch", config.centrality_branch);
      config.event_plane_branch = ReadOptionalString(*table, "event_plane_branch", config.event_plane_branch);
      config.pid_branch =
          ReadOptionalString(*table, "pdg_branch", ReadOptionalString(*table, "pid_branch", config.pid_branch));
      config.px_branch = ReadOptionalString(*table, "px_branch", config.px_branch);
      config.py_branch = ReadOptionalString(*table, "py_branch", config.py_branch);
      config.pz_branch = ReadOptionalString(*table, "pz_branch", config.pz_branch);
      config.mass_branch = ReadOptionalString(*table, "mass_branch", config.mass_branch);
  ...
  ApplicationConfig LoadApplicationConfig(const std::string &path) {
    ...
    config.input_schema = ParseInputSchema(ReadRequiredString(input, "schema", "input"));
    ...
    ParseBlastwaveInputConfig(GetOptionalTable(input, "blastwave"), config.analysis.input.blastwave);

    config.output_root_path = ReadRequiredString(output, "output_root", "output");
    config.analysis.top_directory_name =
        ReadOptionalString(output, "top_directory_name", config.analysis.top_directory_name);
    config.analysis.r2_summary_directory_name =
        ReadOptionalString(output, "r2_summary_directory_name", config.analysis.r2_summary_directory_name);

    if (const toml::table *event_plane = root["event_plane"].as_table(); event_plane != nullptr) {
      ...
      config.analysis.event_plane.harmonic_order =
          ReadOptionalInt(*event_plane, "harmonic_order", config.analysis.event_plane.harmonic_order);
      config.analysis.event_plane.use_internal_reconstruction = ReadOptionalBool(
          *event_plane, "use_internal_reconstruction", config.analysis.event_plane.use_internal_reconstruction);
      config.analysis.event_plane.fallback_to_input_branch = ReadOptionalBool(
          *event_plane, "fallback_to_input_branch", config.analysis.event_plane.fallback_to_input_branch);
      ...
      config.analysis.event_plane.weight_mode = ParseEventPlaneWeightMode(
          ReadOptionalString(*event_plane, "weight_mode", ToString(config.analysis.event_plane.weight_mode)));
      config.analysis.event_plane.allowed_abs_pdg =
          ReadOptionalIntArray(*event_plane, "allowed_abs_pdg", config.analysis.event_plane.allowed_abs_pdg);
      config.analysis.event_plane.min_candidates =
          ReadOptionalSize(*event_plane, "min_candidates", config.analysis.event_plane.min_candidates);
      config.analysis.event_plane.min_q_magnitude =
          ReadOptionalDouble(*event_plane, "min_q_magnitude", config.analysis.event_plane.min_q_magnitude);
    }

    if (const toml::table *selection = root["selection"].as_table(); selection != nullptr) {
      ...
      config.analysis.selection.femto_eta_min =
          ReadOptionalDouble(*selection, "femto_eta_min", config.analysis.selection.femto_eta_min);
      config.analysis.selection.femto_eta_max =
          ReadOptionalDouble(*selection, "femto_eta_max", config.analysis.selection.femto_eta_max);
      config.analysis.selection.femto_pt_min =
          ReadOptionalDouble(*selection, "femto_pt_min", config.analysis.selection.femto_pt_min);
      config.analysis.selection.femto_pt_max =
          ReadOptionalDouble(*selection, "femto_pt_max", config.analysis.selection.femto_pt_max);
      config.analysis.selection.femto_mt_reference_mass =
          ReadOptionalDouble(*selection, "femto_mt_reference_mass", config.analysis.selection.femto_mt_reference_mass);
      config.analysis.selection.femto_mt_mass_tolerance =
          ReadOptionalDouble(*selection, "femto_mt_mass_tolerance", config.analysis.selection.femto_mt_mass_tolerance);
    }

    if (const toml::table *histograms = root["histograms"].as_table(); histograms != nullptr) {
      config.analysis.histograms.warn_on_overflow =
          ReadOptionalBool(*histograms, "warn_on_overflow", config.analysis.histograms.warn_on_overflow);
      if (const toml::table *axis = GetOptionalTable(*histograms, "rho_out_axis"); axis != nullptr) {
        config.analysis.histograms.rho_out_axis =
            ParseAxisSpec(*axis, config.analysis.histograms.rho_out_axis, "histograms.rho_out_axis");
      }
      if (const toml::table *axis = GetOptionalTable(*histograms, "rho_side_axis"); axis != nullptr) {
        config.analysis.histograms.rho_side_axis =
            ParseAxisSpec(*axis, config.analysis.histograms.rho_side_axis, "histograms.rho_side_axis");
      }
      if (const toml::table *axis = GetOptionalTable(*histograms, "rho_long_axis"); axis != nullptr) {
        config.analysis.histograms.rho_long_axis =
            ParseAxisSpec(*axis, config.analysis.histograms.rho_long_axis, "histograms.rho_long_axis");
      }
      if (const toml::table *axis = GetOptionalTable(*histograms, "projection_axis"); axis != nullptr) {
        config.analysis.histograms.projection_axis =
            ParseAxisSpec(*axis, config.analysis.histograms.projection_axis, "histograms.projection_axis");
      }
    }
    ...
    if (const toml::table *bins = root["bins"].as_table(); bins != nullptr) {
      if (const toml::array *array = GetOptionalArray(*bins, "centrality"); array != nullptr) {
        config.analysis.centrality_bins = ParseRangeBinArray(array, "bins.centrality");
      }

      if (const toml::array *array = GetOptionalArray(*bins, "mt"); array != nullptr) {
        config.analysis.mt_bins = ParseRangeBinArray(array, "bins.mt");
      }

      if (const toml::array *array = GetOptionalArray(*bins, "phi"); array != nullptr) {
  ...
  void ValidateApplicationConfig(ApplicationConfig &config) {
    ...
    ValidateNonEmpty("output.r2_summary_directory_name", config.analysis.r2_summary_directory_name);

    ValidateLegacyInputConfig(config.analysis.input.legacy);
    ValidateBlastwaveInputConfig(config.analysis.input.blastwave);

    if (config.analysis.selection.femto_eta_max <= config.analysis.selection.femto_eta_min) {
      throw ConfigError("selection.femto_eta_max must be greater than femto_eta_min.");
    }
    if (config.analysis.selection.femto_pt_max <= config.analysis.selection.femto_pt_min) {
  ...
  void ApplyCliOverrides(const CliOptions &cli_options, ApplicationConfig &config) {
    ...
    if (cli_options.input_schema_override.has_value()) {
      const EventPlaneConfig current_default = MakeDefaultEventPlaneConfigForSchema(config.input_schema);
      if (config.analysis.event_plane.use_internal_reconstruction == current_default.use_internal_reconstruction
          && config.analysis.event_plane.fallback_to_input_branch == current_default.fallback_to_input_branch) {
        config.analysis.event_plane = MakeDefaultEventPlaneConfigForSchema(*cli_options.input_schema_override);
      }
