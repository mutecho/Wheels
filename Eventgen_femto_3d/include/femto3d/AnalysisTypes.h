...
struct SliceKey {
  ...
  [[nodiscard]] bool operator<(const SliceKey& other) const {
    return std::tie(cent_index, mt_index, phi_index) <
           std::tie(other.cent_index, other.mt_index, other.phi_index);
  }
};
...
inline std::string FormatBinLabel(const std::string& prefix,
  ...
  stream << prefix << "_" << std::fixed << std::setprecision(precision) << min
         << "_" << max;
  return stream.str();
}
...
inline double ProjectOntoDirection(const PairSeparationLCMS& separation,
                                   const ProjectionDefinition& direction) {
  return direction.n_out * separation.rho_out +
         direction.n_side * separation.rho_side +
