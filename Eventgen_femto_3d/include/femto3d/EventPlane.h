...
namespace femto3d {
  ...
  [[nodiscard]] bool IsEventPlaneCandidate(const ParticleState &particle, const EventPlaneConfig &config);

  [[nodiscard]] EventPlaneResult ReconstructEventPlane(const std::vector<ParticleState> &event_plane_candidates,
                                                       const EventPlaneConfig &config);

  [[nodiscard]] EventPlaneResult MakeInputEventPlaneResult(double event_plane_psi, int harmonic_order);

  [[nodiscard]] double WrapAngleToEventPlanePeriod(double angle, int harmonic_order);

  [[nodiscard]] double WrapPhiMinusPsi2(double angle);

  [[nodiscard]] double ComputePairPhiMinusEventPlane(const PairKinematics &pair_kinematics,
                                                     double event_plane_psi,
                                                     int harmonic_order);

  [[nodiscard]] double ComputePairPhiMinusPsi2(const PairKinematics &pair_kinematics, double event_plane_psi2);
